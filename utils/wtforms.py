...
from wtforms import EmailField, PasswordField, BooleanField, SubmitField, StringField, DateField, TextAreaField, \
...
class LoginForm(FlaskForm):
    email = EmailField('Email', validators=[DataRequired(), Email(msg['email'])], description="example@company.com",)
    password = PasswordField('Пароль', validators=[DataRequired(), Length(min=6, max=15, message=msg['psw'])],
...
class RegisterForm(FlaskForm):
    firstName = StringField('Имя:', validators=[DataRequired(), Length(max=50)], description='Иван')
    ...
    password = PasswordField('Пароль', validators=[DataRequired(), EqualTo('repeatPassword', message=''),
                             Length(min=6, max=15, message=msg['psw'])], description="example123")

    repeatPassword = PasswordField('Подтвердите пароль',
                                   validators=[DataRequired(), EqualTo('password', message='Пароли не совпадают'),
                                               Length(min=6, max=15, message=msg['psw'])],
                                   description="example123")
...
class EventCreateForm(FlaskForm):
    ...
    venueDescription = TextAreaField('Описание:', id='venueDescription', name='venueDescription',
    ...
    seats = IntegerField('Кол-во сидячих мест:', id='seats', name='seats', validators=[NumberRange(min=0)],
                         default=0)
...
class OrganizerCreateForm(FlaskForm):
    ...
    def validate_cardHolderName(form, field):
